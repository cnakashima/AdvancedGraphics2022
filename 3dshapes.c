...
int Texture(double color[3], int texture_code, double u, double v, double lowu, double upu, double lowv, double upv, double texture[3]){
	...
	if (texture_code == 1){
		...
		if ((((int)((u+lowu)/box_width))%2) ==  (((int)((v+lowv)/box_height))%2)){
			texture[0] = 1.0;
			texture[1] = 1.0;
			texture[2] = 1.0;
		}
...
int graph_function_3d(double (*fx)(double u, double v), double (*fy)(double u, double v), double (*fz)(double u, double v),
      ...
      double lowboundv,
      double upboundv,
	    double move[4][4], double color[3], double step){
  // printf("check\n");
  ...
  for (double u = lowboundu; u < upboundu; u += step){
    ...
    double texture[3];
    Texture(color, 1, u, v, lowboundu, upboundu, lowboundv, upboundv, texture);
...
// cylinder
  nl = 0 ;
  tlist[nl] = SX ; plist[nl] = .1 ; nl++ ;
  tlist[nl] = SZ; plist[nl] =  .1 ; nl++ ;
  ...
  graph_function_3d(cylinderx, cylindery, cylinderz, 0, 2*M_PI, -2.0, 2.0, move, color, .005*M_PI);
  ...
  graph_function_3d(spherex, spherey, spherez, 0, 2*M_PI, -.5*M_PI, .5*M_PI, move, color, .005*M_PI);
