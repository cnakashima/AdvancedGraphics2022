...
int ray (double* Rsource, double * Rtip, double* argb, int depth, int flag);

// To support the light model (taken from view_test05MbS.c by Jeff) :
double light_in_eye_space[3] = {0.0, 0.0, 10.0};
double AMBIENT      = 0.2 ;
...
// angle between vectors used for refraction
double angle_between_vectors(double a[3], double b[3]){
	double adotb = a[0]*b[0] + a[1]*b[1] + a[2]*b[2];
	double maga = sqrt((a[0]*a[0]) + (a[1]*a[1]) + (a[2]*a[2]));
	double magb = sqrt((b[0]+b[0]) + (b[1]*b[1]) + (b[2]*b[2]));
	return acos(adotb/(maga*magb));
} 
	...
	for (int i = 0; i < num_objects; i++){
		...
		if (pt_found > -1){
			...
			if (reflection[pt_found] > 0.0 && depth > 0){
				...
				if (!ray(source, tip, lrgb, depth - 1, 1)){
					lrgb[0] = 0;
					lrgb[1] = 0;
					lrgb[2] = 0;
				}
				...
				argb[0] = (1 - reflection[pt_found]) * argb[0] + reflection[pt_found] * lrgb[0];
				argb[1] = (1 - reflection[pt_found]) * argb[1] + reflection[pt_found] * lrgb[1];
				argb[2] = (1 - reflection[pt_found]) * argb[2] + reflection[pt_found] * lrgb[2];
			}
